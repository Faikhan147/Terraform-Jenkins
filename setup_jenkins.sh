...
sudo aws s3 cp s3://terraform-backend-all-environments/Jenkins-Authentication/jenkins-login.yaml  /var/lib/jenkins/casc_configs/

# Copy Jenkins Plugins  YAML file from S3 folder to Jenkins config folder
sudo aws s3 cp s3://terraform-backend-all-environments/Jenkins-Plugins/  /var/lib/jenkins/init.groovy.d/ --recursive
...
sudo aws s3 cp s3://terraform-backend-all-environments/Jenkins-Credentials/ /var/lib/jenkins/casc_configs/ --recursive

# Copy SonarQube Authentication YAML file from S3 folder to Jenkins config folder
sudo aws s3 cp s3://terraform-backend-all-environments/Jenkins-System/sonar-authentication.yaml /var/lib/jenkins/casc_configs/

# Copy SonarQube Scanner YAML file from S3 folder to Jenkins config folder
sudo aws s3 cp s3://terraform-backend-all-environments/Jenkins-Tools/sonar-scanner.yaml /var/lib/jenkins/casc_configs/

# Copy seed Job  YAML file from S3 folder to Jenkins config folder
sudo aws s3 cp s3://terraform-backend-all-environments/Jenkins-Pipelines/seed-job /var/lib/jenkins/init.groovy.d/ --recursive
